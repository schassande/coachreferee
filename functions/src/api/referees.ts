...
export let refereesRouter = express.Router();

// GET /api/referees/:id
refereesRouter.get("/:id", async function getReferee(req: express.Request, res: express.Response) {
...
// GET /api/referees?region=xxx&country=xxx&shortName=xxx&shortNames=xxx,xxx&level=xxx&email=xxx
refereesRouter.get("*", async (req: express.Request, res: express.Response) => {
    if (!tool.ensureApiKey(req, res)) return;
    const firestore: admin.firestore.Firestore = admin.firestore();
    ...
    if (req.query.level) {
        query = query.where('referee.refereeLevel', '==', req.query.level);
    }
    if (req.query.email) {
    ...
    elems = elems.filter((elem) => elem.applications.filter(
        (ar:any) => ar.name === 'RefereeCoach' && ar.role === 'REFEREE').length > 0);
